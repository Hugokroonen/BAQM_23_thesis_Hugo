...
assert MODEL in ['FULL', 'CTM', 'LDA_X'], \
    'Valid options for MODEL argument are FULL, CTM, or LDA_X'
assert M >= 2, \
...
if MODEL == 'FULL':
    ...
    NO_DYNAMICS = True
    NO_REGRESSORS = False
    print('CTM model - without VAR(1) effects')
elif MODEL == 'LDA_X':
    EMULATE_LDA_X = True
    NO_DYNAMICS = False
    NO_REGRESSORS = False
...
# Create a dataset, based on the (y_fused_ibn, x, h)-data
