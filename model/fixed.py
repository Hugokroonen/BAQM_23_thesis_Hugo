...
# Defining namedtuples for IsFixed and FixedValues structures
...
def create_fixed(
    ...
    if emulate_lda_x:
        ...
        This results in:
            alpha_i1 ~ Normal_M(mu=delta + gamma @ h_i, sigma_sq=tau_alpha^-1)
        ...
        The other parts of the model should be fixed as well:
            ...
            rho = 0-matrix
        """
        # Emulate LDA-X behavior by fixing specific model parameters
        # Set parameter values for emulation
    ...
    # Handle no_regressors scenario
    if no_regressors:
        beta = np.zeros((M, dim_x))
        delta_beta = np.zeros(1)

        gamma = np.zeros((M, dim_h))
