"""
Description:
    Contains the Prior structure for the CTM model.

Functions:
    create_prior: creates the prior structure for the CTM model based on
...
def create_prior(
    ...
    phi_dist = dirichlet
    phi_alpha = np.ones((dim_j, M)) / dim_j # Matrix (# products, # motivations) with each element 1/dim_j

    # p(tau_alpha) ~ Gamma_M(alpha, beta)
    # Note: This is a Gamma __vector__
    tau_alpha_dist = gamma_v # Gamma 
    tau_alpha_alpha = np.ones(M) # alpha = 1 
    tau_alpha_beta = np.ones(M) # beta = 1 

    # p(mu_kappa) ~ MVN_M(mu, Sigma)
    mu_kappa_dist = mvn # Multivariate Normal 
    mu_kappa_mu = np.zeros(M) # 0 array 
    mu_kappa_sigma = np.identity(M) # represents the covariance matrix of the multivariate normal distribution. Identity matrix. 

    # p(lambda_kappa) ~ Wishart_M(n, V)
    lambda_kappa_dist = wishart # Wishart
    lambda_kappa_n = 2 * M # 2M
    lambda_kappa_v = np.identity(M) / (2 * M) # identity matrix / 2M 

    # p(beta_m) ~ MVN_{dim_x}(mu, Sigma)
    # shopping trip variables coefficients
    beta_dist = mvn 
    beta_mu = np.zeros(dim_x) # 0 array, with dimension being the number of trip-specific variables 
    beta_sigma = np.identity(dim_x) # identity matrix 

    # p(gamma_m) ~ MVN_{dim_w}(mu, Sigma)
    # customer characteristic variables coefficients
    gamma_dist = mvn
    gamma_mu = np.zeros(dim_h) # 0 array, with dimension being the number of customer-specific variables 
    gamma_sigma = np.identity(dim_h) # identity matrix

    # p(rho_m) ~ MVN_M(mu, Sigma)
    # for the lagged VAR term 
    rho_dist = mvn
    rho_mu = np.zeros(M) 
    rho_sigma = np.identity(M) # MxM matrix 

    # p(delta) ~ Normal_M(mu, sigma_sq)
    delta_dist = normal_v # normal distribution 
    delta_mu = np.zeros(M) # 1 dimensional arrays 
